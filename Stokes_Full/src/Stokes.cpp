  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      for (unsigned int q = 0; q < n_q; ++q)
        {
          const double nu_val = nu(fe_values.quadrature_point(q));
          const double sigma_val = sigma(fe_values.quadrature_point(q));
          const Tensor<1, dim> f_val = f(fe_values.quadrature_point(q));
