...
public:
  ...
  Stokes(const std::string  &mesh_file_name_,
         ...
         const std::function<double(const Point<dim> &)> &nu_,
         const std::function<double(const Point<dim> &)> &sigma_,
         const std::function<Tensor<1, dim>(const Point<dim> &)> &f_,
         const std::function<Tensor<1, dim>(const Point<dim> &)> &h_)
...
protected:
  ...
  const std::function<double(const Point<dim> &)> nu;

  // sigma * u
  const std::function<double(const Point<dim> &)> sigma;

  // Forcing term
  const std::function<Tensor<1, dim>(const Point<dim> &)> f;

  // Boundary condition
  const std::function<Tensor<1, dim>(const Point<dim> &)> h;
