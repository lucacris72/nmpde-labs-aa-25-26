...
public:
  ...
  // Constructor.
  StokesTime(const std::string  &mesh_file_name_,
         const double       &T_,
         const double       &dt_,
         const double       &theta_,
         const unsigned int &degree_velocity_,
         const unsigned int &degree_pressure_,
         const std::function<double(const Point<dim> &, const double &)> &nu_,
...
protected:
  ...
  const std::function<double(const Point<dim> &, const double &)> nu;

  // sigma * u
  const std::function<double(const Point<dim> &, const double &)> sigma;

  // Forcing term
  const std::function<Tensor<1, dim>(const Point<dim> &, const double &)> f;

  // Boundary condition
  const std::function<double(const Point<dim> &, const double &)> h;
