...
StokesTime::assemble()
{
  ...
  std::vector<Tensor<1, dim>> u_old_values(n_q);

  // Evaluation of the gradient of the old solution on quadrature nodes of
  // current cell.
  std::vector<Tensor<2, dim>> u_old_grads(n_q); // grad u (as Tensor<2>)
  std::vector<double>         p_old_values(n_q);
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      fe_values[velocity].get_function_values(solution, u_old_values);
      fe_values[velocity].get_function_gradients(solution, u_old_grads);
      fe_values[pressure].get_function_values(solution, p_old_values);

      for (unsigned int q = 0; q < n_q; ++q)
        {
          const double nu_val = nu(fe_values.quadrature_point(q), time);
          const double sigma_val = sigma(fe_values.quadrature_point(q), time);
          const Tensor<1, dim> f_old = f(fe_values.quadrature_point(q), time - dt);
          const Tensor<1, dim> f_new = f(fe_values.quadrature_point(q), time);
              ...
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  ...
                  cell_pressure_mass_matrix(i, j) +=
                    fe_values[pressure].value(i, q) *
                    fe_values[pressure].value(j, q) / nu_val * fe_values.JxW(q);
                }

              // Time derivative.
              cell_rhs(i) += (1.0 / dt) * 
                             scalar_product(fe_values[velocity].value(i, q), 
                             u_old_values[q]) * fe_values.JxW(q);  

              // Viscosity term.
              cell_rhs(i) -= (1.0 - theta) * nu_val * 
                             scalar_product(fe_values[velocity].gradient(i, q), 
                             u_old_grads[q]) * fe_values.JxW(q);  

              // sigma * u term.
              cell_rhs(i) -= (1.0 - theta) * sigma_val * 
                             scalar_product(fe_values[velocity].value(i, q), 
                             u_old_values[q]) * fe_values.JxW(q);
                             
              // Pressure term
              cell_rhs(i) += (1.0 - theta) * fe_values[velocity].divergence(i, q) *
                             p_old_values[q] *
                             fe_values.JxW(q);

              // Forcing term.
              cell_rhs(i) += scalar_product(theta * f_new + (1.0 - theta) * f_old, 
...
void
StokesTime::solve_linear_system()
{
  pcout << "===============================================" << std::endl;
...
StokesTime::output()
{
  ...
  const std::string output_file_name = "output-StokesTime";
